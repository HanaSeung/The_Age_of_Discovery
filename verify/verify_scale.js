...
//
// 2026.07 개정 세 가지:
//  1. 경로가 'D:\MyApp\...' 로 박혀 있어 C: 컴퓨터에서는 아예 돌지 않았다 → RELOC_ROOT
//  2. data/land_data.js 이진을 자기 안에서 다시 해석하고 있었다. 형식이 delta8 로 바뀌자
...
console.log(`  world_chart.html ${Math.round(html.length/1024)} KB` +
            ` | data/land_data.js ${Math.round(fs.statSync(path.join(DIR,'data/land_data.js')).size/1024)} KB`);
chk('지도 자료가 HTML 밖에 있다', html.includes('src="data/land_data.js"'));
...
// data/land_data.js 는 window 에 붙으므로 require 하지 않는다 — 머리글만 글로 읽는다
const ldjs = fs.readFileSync(path.join(DIR, 'data/land_data.js'), 'utf8').slice(0, 800);
