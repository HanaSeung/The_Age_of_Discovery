...
// 실제로 실행해 cloudVeil() 을 돌리고, 나온 알파 배열을 data/cloud_data.js 에서
...
// ── 1. 코드 구조 ────────────────────────────────────────────
console.log('\n=== 1. 코드 구조 ===');
chk('data/cloud_data.js 를 읽어들인다', /<script src="data\/cloud_data\.js"><\/script>/.test(html));
...
// ── 2. data/cloud_data.js 를 독립적으로 읽는다 ───────────────────
// world_chart.html 의 CLOUD 모듈을 쓰지 않는다. 같은 코드로 같은 답을 얻으면
// 서로 틀려도 알 수 없다. 여기서 처음부터 다시 푼다.
const cjs = fs.readFileSync(path.join(D, 'data/cloud_data.js'), 'utf8');
...
console.log('\n=== 2. 페이지가 뜨는가 ===');
for(const f of ['data/land_data.js','data/bathy_data.js','data/currents_data.js',
                'data/wind_data.js','data/cloud_data.js'])
