...
new Function('window', 'atob', fs.readFileSync(path.join(DIR, 'data/land_data.js'), 'utf8'))
...
const straits = [
  ...
  // 10m 자료에는 해협 한가운데 페림섬(43.39~43.44E, 12.63~12.68N)이 있다.
  // 50m 자료에는 없던 섬이라 예전 경로(43.32,12.58 → 43.48,12.72)는 섬을 관통했다.
  // 실제 항로대로 섬 서쪽 수로를 남북으로 지난다 — 최소여유 2.6km.
  ['바브엘만데브', 43.35,12.50,  43.35,12.80],
];
let opened = 0;
...
for(let t=0;t<400;t++){
  ...
  for(const sgn of [1,-1]){
    ...
    // 10m 자료는 해안 굴곡이 2km 안팎이라, 바다 쪽으로 1.47km 곧게 나아가는 길이
    // 실제로 막혀 있는 자리가 흔하다 (작은 만·좁은 수로). 그런 곳을 못 지나는 것은
    // 충돌이 옳게 막는 것이므로 시험 대상이 아니다 — 길이 뚫린 경우만 센다.
    // (50m 자료 때는 선분이 11km 라 이런 일이 드물어 가정이 성립했다.)
    let clear = true;
    for(let k=1;k<=24 && clear;k++){
      nearestCoast(px + nx*sgn*0.30*(k/24), py + ny*sgn*0.30*(k/24), 0.5, E);
      if(E.d < HULL_R) clear = false;
    }
    if(!clear) continue;
    et++;
