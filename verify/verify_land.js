// verify_land.js — data/land_data.js 델타 형식과 디코더 검증
...
console.log(`  고리 ${L.rings} | 점 ${L.points} | scale ${L.scale} | 형식 ${L.format}` +
            ` | .js ${(fs.statSync(path.join(DIR,'data/land_data.js')).size/1024).toFixed(0)} KB`);
...
const GJ = path.join(DIR, 'raw', 'ne_10m_land.geojson');
if(!fs.existsSync(GJ)){
  console.log('  --   raw\\ne_10m_land.geojson 이 없어 건너뜀 (python build\\build_land.py 로 내려받는다)');
} else {
...
console.log('\n=== 5. 생성기 규약 ===');
const py = fs.readFileSync(path.join(DIR, 'build', 'build_land.py'), 'utf8');
