...
console.log('\n=== 1. 데이터 파일 ===');
for(const f of ['data/land_data.js','data/bathy_data.js','data/currents_data.js','data/wind_data.js',
                'data/cloud_data.js']){
...
console.log('\n=== 6. 검증들이 기계를 가리지 않는가 ===');
{
  ...
  const VDIR = px.join(RELOC_ROOT, 'verify');   // 검증 스크립트는 verify\ 에 산다
  const files = fsx.readdirSync(VDIR).filter(f => /^verify_.*\.js$/.test(f));
  const bad = [];
  for(const f of files){
    const t = fsx.readFileSync(px.join(VDIR, f), 'utf8');
  ...
  chk('절대경로를 박은 곳이 없다 — 두 기계에서 같이 돈다', bad.length === 0,
      bad.length ? bad.join(' / ') : 'RELOC_ROOT 기준으로만 연다');
}
